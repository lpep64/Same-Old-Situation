...
class MuJoCoXMLBuilder:
    ...
    def add_sites_to_robot_body(self, body_name: str, sites: List[Dict[str, str]]) -> 'MuJoCoXMLBuilder':
        """Add sites to a specific robot body.
        
        Args:
            body_name: Name of the robot body to modify
            sites: List of site definitions (each with name, pos, size, rgba, etc.)
            
        Returns:
            Self for method chaining
        """
        # Find and modify the body in worldbody_elements
        for i, element in enumerate(self.worldbody_elements):
            if element.get("_xml_tag") == "body_xml" and "content" in element:
                content = element["content"]
                
                # Parse the XML content to find the target body
                try:
                    body_root = ET.fromstring(content)
                    if body_root.tag == "body" and body_root.get("name") == body_name:
                        # Add sites to this body
                        for site_def in sites:
                            site_elem = ET.SubElement(body_root, "site")
                            for attr, value in site_def.items():
                                site_elem.set(attr, value)
                        
                        # Update the content
                        self.worldbody_elements[i]["content"] = ET.tostring(body_root, encoding='unicode')
                        break
                    else:
                        # Check if this body contains the target body as a child
                        target_body = self._find_body_in_tree(body_root, body_name)
                        if target_body is not None:
                            # Add sites to the found body
                            for site_def in sites:
                                site_elem = ET.SubElement(target_body, "site")
                                for attr, value in site_def.items():
                                    site_elem.set(attr, value)
    ...
    def _find_body_in_tree(self, root_elem, body_name: str):
        """Recursively find a body with the given name in the XML tree."""
        if root_elem.tag == "body" and root_elem.get("name") == body_name:
            return root_elem
        
        for child in root_elem:
            if child.tag == "body":
                result = self._find_body_in_tree(child, body_name)
