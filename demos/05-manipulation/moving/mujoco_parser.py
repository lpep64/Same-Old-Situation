...
class MuJoCoParserClass(object):
    ...
    def _parse_xml(self):
        ...
        # Map joint indices to qpos addresses (needed because free joints use multiple qpos slots)
        self.rev_joint_qpos_idxs = np.array([self.model.jnt_qposadr[idx] for idx in self.rev_joint_idxs], dtype=np.int32)
        # Map joint indices to velocity/DOF addresses (needed for Jacobian indexing)
        self.rev_joint_dof_idxs = np.array([self.model.jnt_dofadr[idx] for idx in self.rev_joint_idxs], dtype=np.int32)
